...
const RecommendedPlaces: React.FC<Props> = ({ destination, places }) => {
  console.log("📦 places received in RecommendedPlaces:", places);
            ...
            >
              <div className="relative w-full h-48">
                <Image
                  ...
                  alt={place.title?.trim() || `Suggested Place #${index + 1}`}
                  fill
              ...
              <div className="p-5 space-y-2">
                <h3 className="font-bold text-lg text-gray-800">
                  {place.title?.trim() || `Suggested Place #${index + 1}`}
                </h3>
                <p className="text-sm text-gray-600">
                  {place.description ?? 'A recommended spot nearby.'}
                </p>
                ...
                <div className="flex flex-col space-y-2 mt-4">
                  ...
                  <a
                    href={`https://www.google.com/search?tbm=isch&q=${encodeURIComponent(
                      (place.title?.trim() || `Suggested Place #${index + 1}`) + ' Philippines'
                    )}`}
                    target="_blank"
                    rel="noopener noreferrer"
